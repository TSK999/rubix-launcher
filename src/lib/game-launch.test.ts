...
describe("game launch helpers", () => {
  it("uses Steam's browser-safe run protocol for Steam games", () => {
    ...
    expect(
      getGameLaunchTarget({ id: "1", title: "Apex Legends", addedAt: 1, path: "steam://rungameid/1172470", steamAppId: 1172470 }),
    ).toBe("steam://run/1172470");
  });

  it("opens custom protocols with an anchor click", () => {
    const click = vi.spyOn(HTMLAnchorElement.prototype, "click").mockImplementation(() => undefined);
    expect(isExternalProtocol("steam://run/1172470")).toBe(true);
    expect(openExternalProtocol("steam://run/1172470")).toEqual({ ok: true, url: "steam://run/1172470" });
