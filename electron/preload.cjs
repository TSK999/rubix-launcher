...
contextBridge.exposeInMainWorld("rubix", {
  ...
  minecraft: {
    env: () => ipcRenderer.invoke("mc:env"),
    listInstances: () => ipcRenderer.invoke("mc:list-instances"),
    getInstance: (name) => ipcRenderer.invoke("mc:get-instance", { name }),
    createInstance: (payload) => ipcRenderer.invoke("mc:create-instance", payload),
    renameInstance: (from, to) => ipcRenderer.invoke("mc:rename-instance", { from, to }),
    duplicateInstance: (name, newName) => ipcRenderer.invoke("mc:duplicate-instance", { name, newName }),
    deleteInstance: (name) => ipcRenderer.invoke("mc:delete-instance", { name }),
    openInstanceFolder: (name) => ipcRenderer.invoke("mc:open-instance-folder", { name }),
    updateInstance: (name, patch) => ipcRenderer.invoke("mc:update-instance", { name, patch }),
    installMod: (payload) => ipcRenderer.invoke("mc:install-mod", payload),
    uninstallMod: (instance, projectId) => ipcRenderer.invoke("mc:uninstall-mod", { instance, projectId }),
    toggleMod: (instance, projectId, enabled) => ipcRenderer.invoke("mc:toggle-mod", { instance, projectId, enabled }),
    importModpack: (instanceName) => ipcRenderer.invoke("mc:import-modpack", { instanceName }),
    launch: (name) => ipcRenderer.invoke("mc:launch", { name }),
  },
});
