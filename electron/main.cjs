...
function extractSteamAppId(target) {
  const match = /^steam:\/\/(?:run|rungameid)\/(\d+)/i.exec(target);
  return match?.[1] ?? null;
}

function cmdStart(uri) {
...
function getSteamExeCandidates() {
  const candidates = new Set([
    "C:\\Program Files (x86)\\Steam\\steam.exe",
    "C:\\Program Files\\Steam\\steam.exe",
  ]);
  const envCandidates = [process.env.STEAM_PATH, process.env.STEAM_HOME]
    .filter(Boolean)
    .map((p) => path.join(p, "steam.exe"));
  envCandidates.forEach((p) => candidates.add(p));
...
function querySteamExeFromRegistry() {
  return new Promise((resolve) => {
    ...
    const tryNext = () => {
      ...
      execFile("reg.exe", ["query", key, "/v", "SteamExe"], { windowsHide: true }, (err, stdout) => {
        if (!err && stdout) {
          const match = /SteamExe\s+REG_\w+\s+(.+)/i.exec(stdout);
          const exePath = match?.[1]?.trim();
          if (exePath && fs.existsSync(exePath)) return resolve(exePath);
        }
        execFile("reg.exe", ["query", key, "/v", "InstallPath"], { windowsHide: true }, (installErr, installStdout) => {
          if (!installErr && installStdout) {
            const match = /InstallPath\s+REG_\w+\s+(.+)/i.exec(installStdout);
            const installPath = match?.[1]?.trim();
            const exePath = installPath ? path.join(installPath, "steam.exe") : null;
            if (exePath && fs.existsSync(exePath)) return resolve(exePath);
          }
          tryNext();
        });
...
async function launchSteamApp(appId) {
  if (process.platform === "win32") {
    const registrySteamExe = await querySteamExeFromRegistry();
    const steamExe = registrySteamExe ?? getSteamExeCandidates()[0];
    if (steamExe) {
    ...
    // Use the protocol form generated by Steam's own desktop shortcuts first.
    cmdStart(`steam://rungameid/${appId}`);
    return { ok: true, method: "cmd-start-rungameid" };
  }
...
ipcMain.handle("launch-game", async (_evt, target) => {
  ...
  const steamAppId = extractSteamAppId(trimmed);
  if (steamAppId) {
    try {
      return await launchSteamApp(steamAppId);
    } catch (err) {
